...
enum HostPatternNormalizer {
    ...
    static func normalize(_ value: String, allowsWildcard: Bool) -> String? {
        ...
        // strip for anything bracket-wrapped: a trailing dot has no FQDN meaning on an IP
        // literal, so leaving it in place lets the ordinary hasSuffix(".") guard below
        // reject the value outright, exactly as any trailing dot was rejected before this
        // exception existed — no legitimate FQDN starts with "[". Also leave "." and
